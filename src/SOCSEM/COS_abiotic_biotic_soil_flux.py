...
# in pmol / m2 / sec
# Forest 20. (Commane et al., 2015)
# Meredith et al 2018 incubations found highest Q10 numbers for tropical sites (~3)
...
# so we arbitrarily triple the maximum incubation observation
...
def OCS_rainforest_production(soil_temp):
    ...
    return pru_a/(1+pru_b*np.exp(-pru_k*soil_temp)) #in pmol m^-2 sec^-1
...
def OCS_forest_production(soil_temp):
    ...
    return wrc_a/(1+wrc_b*np.exp(-wrc_k*soil_temp)) #in pmol m^-2 sec^-1
...
def OCS_ag_production(soil_temp):
    ...
    return bond_a/(1+ok_b*np.exp(-ok_k*soil_temp)) #in pmol m^-2 sec^-1
...
def OCS_grass_production(soil_temp):
    ...
    return  stunt_a/(1+stunt_b*np.exp(-stunt_k*soil_temp)) #in pmol m^-2 sec^-1

#r2 = 0.64, Mollie Beattie Habitat Community, Port Aransas, TX, Field Measurements
def OCS_wetland_production(soil_temp):
    #y = a at high temperature
    tx_a = 295. #max value recorded by DeLaune et al. (2002)
    tx_k = 0.07855407
    tx_b = 41.16705972
    return tx_a/(1+tx_b*np.exp(-tx_k*soil_temp)) #in pmol m^-2 sec^-1
...
# Estimate max COS uptake, soil volumetric water content (VWC) at max uptake, 
# and uptake at 35% VWC with soil temperature (in C) as input
# 35% VWC was chosen arbitrarily to reduce the number of variables when curve fitting
# and because the observed soil moisture at max uptake is (so far observed) <35
# For these equations
# optimum soil moisture must be less than the arbitrary "other" flux
...
# Like stated above, the role of lichen is woefully ignored
...
#This function is included for posterity and in case you would like
#to do your own curve fitting without choosing the "other" VWC
...
#A fine idea unless you have a reason not to
CONSTANT_PARAM = 35
...
################################################################################
# The shape and behavior of OCS uptake in soil is based on lab incubations
# In van Diest and Kesselmeier 2008, Biogeosciences.
# They found a similar bell-like relationship with temperature 
# at optimum soil water content

# Here, we take the maximum uptake flux observed, 
# add whatever abiotic production might be happening based on the equations above
# and declare that this is as close to the "optimum" uptake that we can estimate
...
# Then we will feed these values into the relationship with soil moisture
...
def grass_opt_uptake(T):#gives us optimum uptake for different temperature curves
    return flux_at_all_theta(T, -4.5,   -1.48268657,  25., 10.86745456) #errors 0.51996858,  0.21108821,  1.87418277 #flux at 25: -0.3223745
def grass_other_uptake(T):
    return flux_at_all_theta(T,  -2.33809598,  -1.27719641,  25., 14.75202332)#errors  0.4362547 ,  0.47732149,  2.71998642
def grass_uptake(T,SW):
    opt_uptake_for_T = grass_opt_uptake(T)
    uptake_at_25_sw = grass_other_uptake(T)
    return flux_at_all_theta(SW,  opt_uptake_for_T,  uptake_at_25_sw,  25., grass_opt_sw())
...
def boreal_opt_sw():
    return 12.5
def boreal_opt_uptake(T):#gives us optimum uptake for different temperature curves
    return flux_at_all_theta(T, -18.24779932, -12.,  35., 28.05488082) # 2.3067844 ,  6.82087416,  2.50783578
def boreal_other_uptake(T):
    return flux_at_all_theta(T, -5.89511395,  -3.76628476,  35., 28.05488082)#25.4438112) # 1.07028805,  2.45884407, 3.45423581
def boreal_uptake(T,SW):
    opt_uptake_for_T = boreal_opt_uptake(T)
    uptake_at_20_sw = boreal_other_uptake(T)
    return flux_at_all_theta(SW,  opt_uptake_for_T,  uptake_at_20_sw,  19.3, boreal_opt_sw())
...
def temperate_opt_sw():
    return 24.5 #average of 3 values, standard deviation 1.0
def temperate_opt_uptake(T):#gives us optimum uptake for different temperature curves
    return -12.6 #Not enough data, the upper limit is declared to be the highest uptake recorded at Wind River
...
def temperate_uptake(T,SW):
    ...
    return flux_theta_g_constant(SW,  opt_uptake_for_T,  uptake_at_51_sw,  temperate_opt_sw())
...
def tropical_other_uptake(T):
    ...
    # other uptake recorded at soil moisture 31 +/0 1.0
def tropical_uptake(T,SW):
    CONSTANT_PARAM = 31.
    opt_uptake_for_T = tropical_opt_uptake(T)
    uptake_at_31_sw = tropical_other_uptake(T)
    return flux_theta_g_constant(SW,  opt_uptake_for_T,  uptake_at_31_sw,  temperate_opt_sw())

#AGRICULTURAL SOIL
#Based on soil incubations from Bondville FLUXNET site (US-Bo1) (Whelan et al. 2016)
#informed by field data from Oklahoma wheatfield (Maseyk et al. 2014)
def ag_opt_sw(): #                                                              Okay
...
def ag_opt_uptake():#gives us optimum uptake for different temperature curves
    ...
    #further evidence that we might be over-correcting for OCS production
def ag_other_uptake():
    return -5.36 #average of bondville soil incubations from 20.9 to 22.2 VWC
    #at various temperatures, "corrected" for production by less_20
def ag_uptake(T,SW):
    CONSTANT_PARAM = 22.
    opt_uptake_for_T = ag_opt_uptake()
    uptake_at_22_sw = ag_other_uptake()
    return flux_theta_g_constant(SW,  opt_uptake_for_T,  uptake_at_22_sw,  ag_opt_sw()) 
